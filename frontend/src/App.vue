<template>
  <div id="app">
        ...
        <div class="modal-dialog modal-lg">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title">Extreme Weather Parameters</h5>
              <button
                type="button"
                class="btn-close"
                aria-label="Close"
    ...
    <transition name="fade">
      ...
      >
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title">Critical Infrastructure</h5>
              <button
                type="button"
                class="btn-close"
                aria-label="Close"
    ...
    <div v-if="showInfoBar" class="info-bar">
      <InfoBar :menu-info="lastRequest" @asset-type-click="onAssetTypeClick" />
    </div>
...
const hbomByUuid = computed(() => {
  const map = {};
  if (hbomDefinitions.value && hbomDefinitions.value.components) {
    // Fixed: components not children
    (function walk(node) {
      map[node.uuid] = node;
...
const submitClimate = async (formData) => {
  ...
  if (!hasPoint && !hasBbox) {
    errorMessage.value =
      "Must select a location on the map or enter coordinates.";
  ...
  try {
    ...
    setTrendAnalysisResults(
      climateData?.climate_analysis?.analysis_results || {}
    );
    ...
    if (Array.isArray(climateData.times) && climateData.times.length) {
      const iso = climateData.times.map(
...
const submitInfrastructure = async () => {
  ...
  try {
    ...
    if (!infraResp.ok)
      throw new Error(`Infrastructure fetch error ${infraResp.status}`);
...
const loadHBOMWithFragility = async () => {
  try {
    ...
    const url = `${apiBaseUrl}/api/fragility/compute/${encodeURIComponent(
      sec
    )}/${encodeURIComponent(haz)}`;
...
const extractFragilityTimeSeries = (hbom) => {
  ...
  const traverse = (node) => {
    ...
    if (hazData?.fragility_curves) {
      ...
      Object.entries(hazData.fragility_curves).forEach(
        ([varName, gridDict]) => {
...
const onAssetSelected = (asset) => {
  const sel =
    hbomByUuid.value[asset.id] ?? hbomByUuid.value[asset.uuid] ?? asset;
...
const handleInfrastructureFile = async (file) => {
  if (!climateBbox.value) {
    console.warn("No bounding box yet; load climate data first.");
    return;
  }
  ...
  try {
    ...
    generateColorMapping();
    markInfrastructureLoaded();
    successMessage.value = "Custom file loaded successfully!";
  } catch (err) {
    console.error("Error uploading file:", err);
    errorMessage.value = `Error uploading or parsing the file: ${err}`;
  }
};
...
const onAssetTypeClick = (type) => {
  ...
  const hasHbom = hbomDefinitions.value?.components?.find(
    (c) =>
      c.label === type ||
